#!/usr/bin/env python3
"""Generate docs/lineage.svg: the three fully-logged own-lineage chains.
...
Chain C, separate again and the frontier's depth-5 point: from a random 146-gate
construction to the 88 @ depth 5, in 64 minutes on a later restart of that same
worker, in a later session, from a different root.

Every step has a timestamped log line and a verified circuit in the run
archives. Data: ../evidence/RESULTS.md sections 3-4 (chain A), 6 (chain B) and
7 (chain C), i.e. the archived worker logs in
../evidence/campaign87_run_2026-07-26_got_88at7/runs_hunt/w10_sym94.log,
../evidence/campaign87_run_2026-07-28_got_88at6_fromscratch/runs_hunt/c_naive.log
and ../evidence/campaign87_run_2026-07-30_got_88at5_fromscratch/runs_hunt/c_naive.log
-- the second of those being a byte-exact prefix of the third.

Provenance note kept in the caption: no chain contains imported material, and the
two derived circuits (88 @ depth 8 and the derived 88 @ depth 5) are deliberately
NOT drawn here -- both of their seed chains pass through Jean's published 88
(ePrint 2026/1481), and RESULTS.md sections 5 and 8 spell them out link by link.
88 is Jean's published count and Jean has priority; chain C removes this
project's dependence on his circuit at depth 5, it does not beat it.
...
CHAIN_C = [
    ("scratch", "", 5, "start"),
    ("146@3", "naive#1958", 5, "step"),
    ("93 &#8594; 89", "walk + lns chunks", 5, "step"),
    ("88@6", "it = 33 873", 5, "step"),
    ("88@5", "it = 37 155", 5, "record"),
]
...
ROW_Y = {0: 56, 1: 176, 2: 296}  # box top per row
HEIGHT = 546
CAPTIONS = [
    ...
    "Middle chain: no seed circuit at all - randomized balanced XOR trees over the 32 raw inputs (139 gates at depth 3), reduced to 88 gates in 37 min,",
    "the depth-6 tie-break landing 0.5 s after the depth-7 one (2026-07-28 17:19:45). A distinct 88-gate family, at Jaccard 0.313-0.323 to every other known 88.",
    "Bottom chain: the frontier\'s depth-5 point, and independent again - a different root of the same worker two days later (146 gates at depth 3, 64 min),",
    "the depth-5 tie-break landing 6.1 s after the depth-6 one (2026-07-30 10:48:19). Another distinct family: weighted Jaccard at most 0.1025 to anything known.",
    "88 is Jean\'s published count (ePrint 2026/1481) and Jean has priority. The 88@7 ties it with an independent circuit - 61/88 shared masks - and the",
    "bottom chain removes this project\'s dependence on his circuit at depth 5; neither beats it. Not shown: the seed chains of the 88@8 and of the derived",
    "88@5 both pass through that published 88 - see evidence/RESULTS.md sections 5 and 8.",
]


def main():
    n = max(len(CHAIN_A), len(CHAIN_B), len(CHAIN_C))
    ...
    e = [f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {width} {HEIGHT}" '
         ...
         '<title id="ltitle">The three own-lineage chains: to the 88-gate depth-7 '
         'circuit, to the 88-gate depth-6 circuit found from scratch, and to the '
         '88-gate depth-5 circuit found from scratch that holds the frontier</title>',
         '<desc id="ldesc">Three left-to-right chains of boxes, each box a verified '
         ...
         'rho-squared-symmetric 94 at depth 5 (five gates worse than the 89, '
         ...
         'different root in a later session, built a randomized 146-gate depth-3 '
         'circuit and reduced it through 93, 91, 90 and 89 to 88 gates at depth 6 '
         'and then, six seconds later, depth 5. The 97, the 89 at depth 5, the 88 '
         'at depth 7, the 88 at depth 6 and the 88 at depth 5 are highlighted as '
         'this project\'s headline circuits.</desc>',
         f'<rect width="{width}" height="{HEIGHT}" fill="white"/>']

    for row, chain in ((0, CHAIN_A), (1, CHAIN_B), (2, CHAIN_C)):
        y0 = ROW_Y[row]
    ...
    for j, cap in enumerate(CAPTIONS):
        e.append(f'<text x="{X0}" y="{378 + j*17}" fill="#666666" font-size="11.5">{cap}</text>')
    e.append("</svg>")
