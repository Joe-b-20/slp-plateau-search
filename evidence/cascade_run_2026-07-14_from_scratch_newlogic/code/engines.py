    ...
    while (time_limit is None or time.time() - t0 < time_limit) and \
        ...
        if accept:
            ...
            if len(Ucur) <= len(Ubest):
                cand = _peel(Ucur, rng, cap)
                Ucur = cand
                if len(cand) < len(Ubest):
                    Ubest = list(cand)
                    ctx.improve(indexpairs_from_masks(cand, cap), set(cand),
                                note="it=%d" % iters)
                elif len(cand) == len(Ubest):
                    if ctx.improve(indexpairs_from_masks(cand, cap), set(cand),
                                   note="depth-tiebreak it=%d" % iters):
                        Ubest = list(cand)      # keep the shallower equal-gate set
...
def engine_walk(start_masks, cap, target, time_limit, seeds, k, ctx):
    ...
    while (time_limit is None or time.time() - t0 < time_limit) and \
        ...
        if len(cur) < len(best):
            ...
            if ctx.improve(indexpairs_from_masks(cur, cap), set(cur), note="depth-tiebreak it=%d" % iters):
                best = set(cur)
        if iters % 20000 == 0:
